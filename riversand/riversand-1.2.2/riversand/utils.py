...
def restandardize(item) -> dict:
    """
    Generate dict of nuclide data restandardized to 07KNSTD/KNSTD.
    mandatory keys 'N', 'delN'
    Sets default values for 'nuclide' and 'standardization' if not specified.
    ...
    if isinstance(item, pd.Series):
        ...
        out = deepcopy(item)
    elif isinstance(item, pd.DataFrame) and len(item)==1:
        out = item.iloc[0].to_dict()
    else:
        raise TypeError("restandardize() 'item' must be dict or pandas Series")
        
    if not all(key in out.keys() for key in ('N', 'delN')):
        raise ValueError("restandardize() required keys ('N', 'delN') missing")
    
    # set default values for nuclide and standardization if not specified
    if 'nuclide' not in out.keys():
        out['nuclide'] = 'Be-10'
        if 'standardization' in out.keys():
            raise ValueError("restandardize() standardization and nuclide must be specified")
                
    if 'standardization' not in out.keys():
        try:
            out['standardization'] = Params._default_standards[out['nuclide']]
        except:
            raise ValueError("restandardize() nuclide must be 'Be-10' or 'Al-26'")
            
    if out['nuclide']=='Be-10':
        if out['standardization'] not in Params.Be_stds.keys():
            raise ValueError("restandardize() illegal Be standardization")
        out['N'] = Params.Be_stds[out['standardization']]*out['N']
        out['delN'] = Params.Be_stds[out['standardization']]*out['delN']
        out['standardization'] = Params._default_standards['Be-10']
    elif out['nuclide']=='Al-26':
        if out['standardization'] not in Params.Al_stds.keys():
            raise ValueError("restandardize() illegal Al standardization")
        out['N'] = Params.Al_stds[out['standardization']]*out['N']
        out['delN'] = Params.Al_stds[out['standardization']]*out['delN']
        out['standardization'] = Params._default_standards['Al-26']
