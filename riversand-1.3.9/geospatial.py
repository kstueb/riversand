...
from rasterio.warp import calculate_default_transform, aligned_target, reproject 
...
class CRSError(Exception):
    def __init__(self, message, crs=None):
    ...
    def __str__(self):
        if self.crs:
            return "{}\n\n{}".format(self.message, self.crs.__repr__())
        else:
...
def get_geotiff(fname:str) -> rasterio.DatasetReader:
    """
    Get closed file handle to geotiff.

    Raises :
        CRSError : Dataset has no geotransform (e.g. if file is not a geotiff)
    ...
    try:
        ...
        raise CRSError(e.args[0]) # 'Dataset has no geotransform, gcps, or rpcs. The identity matrix will be returned.'
...
class Raster():
    ...
    Raises :
        TypeError : "Invalid Raster dtype...", "Invalid Raster fname..."
        CRSError :  "Elevation raster has no coordinate reference system" ('elevation' only)
                    "Coordinate reference system is geographic; projected crs is needed"
                    "Cannot convert crs to epsg"
        IOError :   "File not found or file is not a geotiff"
    ...
    def __init__(self, fname:str, dtype:str):
        ...
        try:
            src = get_geotiff(fname) # returns closed rasterio.DatasetReader
            # CRSError : Dataset has no geotransform (e.g. if file is not a geotiff)
            # RasterioIOError : No such file or directory
        ...
        if src:
            ...
            if dtype=='elevation': # missing crs in elevation raster
                if self.crs is None:
                    raise CRSError("Elevation raster has no coordinate reference system")
            if self.crs is None: # other rasters may lack crs
                pass
            elif self.crs.is_projected==False: # if crs defined it must be projected
                raise CRSError("Coordinate reference system is geographic; projected crs is needed",
                               self.crs)
            elif self.epsg is None: # crs is defined but cannot be converted to epsg
                raise CRSError("Cannot convert crs to epsg",
                               self.crs)
            ...
            raise IOError("File not found, or file is not a geotiff\n{}".format(fname))
...
class Catchment():
    ...
    Raises :
        TypeError : "Invalid Catchment fname..."
        ValueError : "Invalid shapefile geometry...", "Shapefile empty, must have at least 1 polygon"
        CRSError :  "Coordinate reference system is geographic; projected crs is needed"
                    "Cannot convert crs to epsg"
        IOError :   "Cannot read shapefile"
    ...
    def set_cid(self, cid:str='id'):
        ...
        if cid is None:
            ...
            #if self.attrs is None: # ToDo: Exception to be raised by Catchments()
...
class Riversand():
    ...
    def add_raster(self, fname:str, dtype:str='elevation', full_path=None):
        """ Add raster dataset to the project. """
        """
        Print Warnings if 'shielding' or 'quartz' raster does not match existing
        'elevation' raster but adds them regardless. crs and epsg may be None for
        'shielding' and 'quartz' but 'elevation' is guaranteed to have a valid
        (projected) crs and epsg.
        
        The parameter full_path is used by the method self.reproject()
        self.add_raster('', 'elevation',
        ...
        #             "Coordinate reference system is geographic; projected crs is needed"
        #             "Cannot convert crs to epsg"
        # IOError :   "File not found or file is not a geotiff" (if exception raised by get_geotiff())
            
        try:
            R_new = Raster(fname, dtype)
        except CRSError as e: # missing crs for 'elevation', or invalid crs (not projected, no epsg)
            print(type(e).__name__, ":", e.__str__())
        except Exception as e: # TypeError, IOError
            print(type(e).__name__, ":", e.args[0])
        else:
            if dtype=='elevation':
                self.elevation = R_new
                    
            if dtype=='shielding':
                self.shielding = R_new
    ...
    def add_catchments(self, fname:str):
        ...
        try:
            C_new = Catchment(fname)
        except CRSError as e: # invalid crs (not projected, no epsg)
            print(type(e).__name__, ":", e.__str__())
        except Exception as e: # TypeError, ValueError, IOError
            print(type(e).__name__, ":", e.args[0])
        else:
            self.catchments = C_new
    ...
    def set_cid(self, cid:str='id'):
        ...
        if self.catchments is None:
            ...
            #if self.catchments.attrs is None: # ToDo: Exception to be raised by Catchments()
            #    print("ValueError : Invalid cid='{}';\n".format(cid) +
            #          "   shapefile has no attribute fields")
            #    self.cid = None
    ...
    def add_samples_from_file(self, fname:str):
        ...
        for d in set_default:
            if d in Params.default_values.keys(): # excluding standardization
                if isinstance(Params.default_values[d], str): # recast column to dtype object
                    self.samples = self.samples.astype({d: str})
                self.samples[d] = Params.default_values[d]
        if 'standardization' in set_default:
            self.samples = self.samples.astype({'standardization': str}) # recast column to dtype object
    ...
    def reproject(self, out_path:str='rv_reprojected'):
        """
        Reproject and warp all raster datasets to match the projection and
        resolution of 'elevation' (target aligned pixels for elevation).
        
        Files are written to 'rv_reprojected' in current working directory.
        If a different location is specified in parameter out_path any files
        rv_elevation.tif, rv_shielding.tif, rv_quartz.tif will be overwritten.
        
        Riversand object is updated with new raster datasets.
        
        Note that all tifs are converted to float32 (or float64) and nodata is
        set to nan. All Rasters are clipped to the extent of the elevation raster.
        
        Raises :
            ReprojectionFailure : This should not happen
            
        """
    
        def write_reprojected(src_meta, data,
                              dst_crs,
                              transform, width, height,
                              fname:str):
            """
            Reproject data from src_meta['transform'] and src_meta['crs']
            to dst_crs and transform.
            
            Source crs can be different from dst_crs but cannot be None!
            
            src_meta, data : data and metadata of source file
            dst_crs : target crs from self.elevation
            transform, width, height : from calculate_default_transform()
            fname : 'rv_elevation.tif', 'rv_shielding.tif', 'rv_quartz.tif'
            """
            ...
            # recast to float and set nodata=np.nan if necessary
            if src_meta['nodata']: # not None
                if kwargs['dtype'] not in ('float32', 'float64'):
                    # use float32 by default but keep float64 if that's what the data is
            ...
            with rasterio.open(fname, 'w', **kwargs) as dst:
                reproject(
                    source        = data,
                    destination   = rasterio.band(dst, 1), # there can only be one band
                    src_transform = src_meta['transform'],
                    src_crs       = src_meta['crs'],
                    dst_transform = transform,
                    dst_crs       = dst_crs,
                    resampling    = Resampling.bilinear)
                    #dst_nodata defaults to src.nodata if masked array or to 0
            return kwargs
        ...
        #expected behaviour:
        # if rv.elevation is not defined write message and end
        # if rv.elevation is defined but does not exist raise RasterioIOError
        # read data and close file; then reproject and write file
        # no problem "reprojecting" itself
        # but error message if reprojected file is opened (eg QGIS) then tried to reproject again
        
        if self.elevation is None:
            print("Cannot reproject if no elevation raster is defined")
            return
        
        # reproject elevation raster (tap)                
        with rasterio.open(self.elevation.src.name, 'r') as src:
            # raises RasterioIOError: rv_reprojected\rv_elevation.tif: No such file or directory
            if src.nodata:
                data = src.read(1, masked=True)
            else:
                data = src.read(1)
        ...
        fname = 'rv_elevation.tif'
        try:
            _ = write_reprojected(src.meta, data,
                                  src.crs, # crs of elevation raster
                                  transform, width, height,
                                  os.path.join(out_path, fname)
                                  )
            print("Writing new file '{}'".format(fname))
        except (PermissionError, OSError):
            print("Cannot write '{}', file is in use by another program".format(fname))
            return
        
        with rasterio.open(os.path.join(out_path, fname), 'r') as target:
            # crs and res of reprojected elevation file
            target_crs = target.crs
            target_res = target.res
    
        # only if reprojection of self.elevation was successful
        if self.shielding:
            with rasterio.open(self.shielding.src.name, 'r') as src:
                # raises RasterioIOError: rv_reprojected\rv_shielding.tif: No such file or directory
                ...
                # calculate_default_transform(); src.crs may be None or different from target_crs
                transform, width, height = calculate_default_transform(
                    src.crs, target_crs, src.width, src.height, 
                    resolution=target_res, *src.bounds)
                
                # target align pixels
                transform, width, height = aligned_target(
                        transform, width, height, target_res)
                
                # handle potentially missing crs
                src_meta = src.meta
                if src_meta['crs'] is None:
                    print("No crs defined for 'shielding', assuming same as 'elevation'")
                    src_meta['crs'] = target_crs
                    
                
            # write reprojected raster
            fname = 'rv_shielding.tif'
            try:
                _ = write_reprojected(src_meta, data,
                                      target_crs, transform, width, height,
                                      os.path.join(out_path, fname)
                                      )
                print("Writing new file '{}'".format(fname))
            except (PermissionError, OSError):
        ...
        # only if reprojection of self.elevation was successful
        if self.quartz:
            with rasterio.open(self.quartz.src.name, 'r') as src:
                # raises RasterioIOError: rv_reprojected\rv_quartz.tif: No such file or directory
                ...
                src_meta = src.meta
                if src_meta['crs'] is None:
                    print("No crs defined for 'quartz', assuming same as 'elevation'")
                    src_meta['crs'] = target_crs       
        ...
        # store reprojected raster to self
        self.add_raster('', 'elevation', full_path=os.path.join(out_path, 'rv_elevation.tif'))
        if self.shielding:
            self.add_raster('', 'shielding', full_path=os.path.join(out_path, 'rv_shielding.tif'))
        if self.quartz:
            self.add_raster('', 'quartz', full_path=os.path.join(out_path, 'rv_quartz.tif'))   
