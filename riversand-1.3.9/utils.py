...
def clip_raster(polygon:dict, src:rasterio.DatasetReader,
    ...
    with rasterio.open(src.name, 'r') as file:

        if not feature_in_raster(polygon, file):
            raise ValueError("clip_raster() : catchment polygon out of bounds")
    
        # read the raster data with rasterio and clip the catchment
        try:
            Z, Z_transform = rasterio.mask.mask(file, [polygon], nodata=np.nan,
                                                crop=True)
            ...
            with MemoryFile() as memfile:
                ...
                file = memfile.open()  # reopen as DatasetReader
                
                Z, Z_transform = rasterio.mask.mask(file, [polygon], nodata=np.nan,
                                                    crop=True)
        
        Z_meta = file.meta
        #Z_bounds = file.bounds
